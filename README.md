...
- 店舗データは、HP制作でも使っているNotionの「en-connect スポット」データベースを流用する（「クーポン対象」にチェックが入った店舗のみアプリに表示）
...
| `NOTION_STORES_ACTIVE_PROPERTY` | 店舗をアプリに掲載するかのチェックボックスのプロパティ名（省略時 `クーポン対象`） |
...
HP制作で使っている「en-connect スポット」データベース（既存の店舗一覧）をそのまま流用している。アプリ用に以下の2プロパティを追加済み。

| プロパティ名 | 型 | 用途 |
| --- | --- | --- |
| クーポン対象 | チェックボックス | チェックが入っている店舗だけがクーポンアプリに表示される |
| クーポン内容 | テキスト | クーポンの内容（例: 「ソフトドリンク1杯無料」） |

店舗名は「名前」、説明は「概要」、住所は「住所」プロパティを使用。画像はNotionページのカバー画像を使う（Filesプロパティは不要）。

新しい店舗をアプリに載せたい場合は、Notion側でその店舗の「クーポン対象」にチェックを入れ、「クーポン内容」を記入するだけでよい（コード変更不要、キャッシュは60秒）。

データベースを使うには、作成したIntegrationをそのデータベース（またはその親ページ）の「コネクト」から接続しておく必要がある。
